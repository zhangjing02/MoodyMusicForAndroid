...
#### 消息推送与实时同步
- **JPush SDK 5.4.0** - 极光推送，实现云端信号触发与前台实时 UI 刷新。
- **Signal over Data 架构** - 推送仅发送刷新信号而非具体内容，确保数据安全与一致性。
- **生命周期感知同步** - 结合 `AppFlags` 维护“脏标记”，实现前台即时同步与后台静默标记。

## 核心功能更新 (v1.1.0)

### 1. 专辑讨论区集成 (LibraryFragment)
- **无感集成**: 原 `AlbumDetailActivity` 逻辑已全面迁移至 `LibraryFragment` 底部，用户向上滑动即可参与讨论。
- **轻量化设计**: 社交模块仅在需要时加载，不影响核心听歌体验。
- **实时评论流**: 支持即时发布评论并自动刷新列表。

### 2. 实时推送同步系统
- **精准投送**: 基于 `album_{ID}` 的标签系统，确保评论刷新信号仅发送给正在查看该专辑的用户。
- **状态追踪**: 引入 `AppFlags` 追踪 `isAlbumDetailVisible` 和 `visibleAlbumId`，完美处理 Fragment 切换时的推送过滤。
- **断网重连补丁**: 通过“脏标记”机制，确保用户从后台回到前台或从离线回到在线时，自动补抓缺失的社交动态。
...
### v1.1.0 (最新)
- **集成专辑社交讨论功能**：在 Library 页面底部实现低侵入式讨论区
- **引入极光推送 (JPush)**：实现评论实时同步架构
- **前后台同步机制**：通过 AppFlags 脏标记实现后台唤醒后的自动刷新
- **动态 Tag 绑定**：基于专辑 ID 和班级 ID 实现精准推送过滤
